...
TaskSchema.method.clockIn = function(user, callback) {
  var self = this;
  // verify user
  if (user._id !== self.user) return callback('Cannot clock in another user');
...
TaskSchema.method.clockOut = function(user, note, callback) {
  var self = this;
  // verify user
  if (user._id !== self.user) return callback('Cannot clock out another user');
...
TaskSchema.statics.create = function(taskData, callback) {
  ...
  Job.findOne({ _id: taskData.job }, function(err, job) {
    if (err) return callback(err);
    if (!job) return callback(new Error('Invalid job ID'));
