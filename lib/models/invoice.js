...
var Job      = require('./job');
var BtcAddr  = require('bitpay/node_modules/bitcore').Address;
...
InvoiceSchema.statics.create = function(user, job, invoiceData, callback) {
  var Invoice = this;
  ...
  function authorizeCreation(next) {
    ...
    if (job.contractor !== user._id) {
      return next(new Error('Cannot create invoice for another contractor'));
    }
    next();
  };
  // validate tasks (if any)
  function validateTasks(next) {
  ...
  function validateInvoicePrice(tasks, next) {
    if (job.pricingType === 'fixed') {
      if (invoiceData.price) {
        ...
        return next(null, tasks, invoiceData.price);
      }
      // default to job rate
      return next(null, tasks, (job.rate - job.amountPaid));
    }
    ...
    next(null, tasks, price);
  };
  // instantiate invoice
  function createInvoice(tasks, price, next) {
...
InvoiceSchema.statics.reconcileTransaction = function(tx, callback) {
  var self = this;
  ...
  async.parallel(
    ...
    function(err, results) {
      if (err) {
        ...
        return log.err('failed to reconcile transaction', err);
      }
      ...
      if (callback) callback(null, reconciled);
