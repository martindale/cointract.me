...
InvoiceSchema.methods.logTransaction = function(tx, output, callback) {
  ...
  var exact = (self.price === util.toSatoshi(output.value));
...
InvoiceSchema.methods.notifyPayment = function(callback) {
...
InvoiceSchema.statics.create = function(user, invoiceData, callback) {
  ...
  function validateInvoicePrice(job, tasks, next) {
    ...
    // aggregate timecards
    var time = 0;
    tasks.forEach(function(task) { time += task.timecard.total });
