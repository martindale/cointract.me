...
JobSchema.methods.addComment = function(user, commentData, callback) {
  ...
  // create comment
  Comment.create(user, job, commentData, function(err, comment) {
    if (err) return callback(err);
...
JobSchema.methods.queueForPublishing = function(listStart, listEnd, callback) {
  ...
  function createBitPayInvoice(next) {
    ...
    if (config.payments.enabled) {
      ...
      return bpClient.on('error', next).on('ready', function() {
    ...
    // otherwise, just go ahead and publish the job
    self.publish({ status: 'complete' }, self.activationCode, callback);
  };
...
JobSchema.methods.publish = function(bitpayInvoice, activationCode, callback) {
  ...
  if (!activationCode || activationCode !== self.activationCode) {
  ...
  if (~validStatuses.indexOf(bitpayInvoice.status)) {
    ...
    return self.save(function(err) {
      ...
      User.findOne({ _id: self.owner }, function(err, user) {
        if (err) return callback(err);
        if (!user) return callback(new Error('Could not find job owner'));
