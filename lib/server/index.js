...
function startRedirector(done) {
  ...
  var redirector = require('http').createServer(express().use(forceSSL));

  redirector.listen(config.server.redirectPort, function() {
...
function startTransactionMonitor(done) {
  ...
  // start the monitor
  transactionMonitor.start(function(err, uri) {
    if (err) return log.err(err);
    log.info('transaction monitor started - using ' + uri);
    done();
  });
  // setup invoice reconcilliation
  transactionMonitor.on('transaction', function(tx) {
